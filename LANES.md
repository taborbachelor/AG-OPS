...
| 2026-08-20 | **M7 is cut along the air/ground line, and two things happen before any code moves.** Plan in `M7-SPLIT-PLAN.md` (delta, TASK-015; planned only, nothing moved). The boundary is not tidiness — it is the standing decision that *guardian belongs onboard*: every monitor runs on the laptop today and goes silent exactly when the link drops. So the question asked of each of the 76 methods is "when the radio dies, does this still need to run?" Measured: the air-side payload is **329 lines, 15%** (guardian runner 226, terrain 74, keepouts 29), tangled through the other 85%. **Prerequisites, both as their own commits before anything is relocated:** (1) `_handle_msg` — 205 lines, 9% of the class — must become a dispatch registry each layer subscribes to; it is the seam every layer reaches into and cannot be assigned to any of them, and doing it second means every layer keeps a private wire into the dispatcher. (2) Six private reads from routers (`_link_state`/`_sysid`/`_vehicle_sysid`/`_capabilities` in `connection.py:207-210`, `_param_sync` in `vehicle.py:169,185`) must get public accessors first, or the split breaks them silently. 48 names are referenced as `vehicle_manager.<name>` externally and all must keep resolving — the façade stays. Strangler order, terrain first and `link`/`state` last; unit green is necessary and NOT sufficient, since locking and link lifecycle live here. | AIR (delta) | — |
...
| 2026-08-20 | **Terrain following works, and it breaks the guardian's airborne test.** Proven live, link dead: over a 25 m descent the aircraft held 25.7-29.1 m above GROUND while its height above HOME fell 23.6 m -- two traces that are only both true under terrain following. But `guardian.py` gates every monitor on `alt >= airborne_alt_m` (5.0 m) where `alt` is height above the LAUNCH point, and this flight measured **4.5 m above home while genuinely 26.8 m above the ground**, so the airspeed/stall and bank monitors gated themselves OFF mid-spray-pass. guardian.py's KANSAS-IS-FLAT note anticipated the inaccuracy; it did not anticipate that the monitors do not degrade, they **switch off** -- and only on terrain-following flights, which is to say on exactly the profile TASK-010 was built to enable. **NOT fixed here:** teaching `airborne` to prefer `terrain_current_height` changes when safety monitors are live on a real airframe, and needs a decision about the fallback when terrain is stale or absent. Flagged for Tabor, pinned by `tests/sitl/test_scenario_terrain.py`. | AIR (alpha) | -- |
| 2026-08-20 | **A simulated dead link must be dead in BOTH directions.** The `gcs_link` fault silenced our heartbeat only, so the vehicle declared a GCS failsafe while our `TERRAIN_DATA` kept flowing over that same radio -- a link that cannot exist. Measured: a terrain scenario that 'cut the link' was still served **280 grids mid-leg**, which would have made 'the aircraft flew on its own cached terrain' an unearned claim. `gcs_link` now takes the GCS off the air for heartbeat AND terrain; the RECEIVE path deliberately stays up so a scenario can still watch the vehicle react. **Anyone adding a new GCS->vehicle transmit path must suppress it here too, or every linkless proof in the suite quietly weakens.** | AIR (alpha) | -- |
| 2026-08-20 | **`fresh_eeprom` does not mean a fresh vehicle: ArduPilot persists its own terrain cache.** It reloads `cwd/terrain/` on the next boot, and a real 2.9 MB `N39W096.DAT` was sitting there. A terrain scenario that does not clear it measures a cache an EARLIER run filled and passes with the `TERRAIN_DATA` service completely broken. Hence `fresh_terrain` on `POST /api/sim/start` (and `h.launch`), **opt-in and off by default** -- re-fetching is rate-limited by ArduPilot to one block per 2 s, so only a test that is actually proving the service should pay it. | AIR (alpha) | -- |
| 2026-08-20 | **The fifth copy of the spray altitude is fixed, and a 3D view is what made it matter.** The default-twins row above recorded `web/src/SprayPlanPreview.jsx` `PREVIEW_ALT_M = 100` as the copy the S3 sweep missed. It was harmless for one reason only: Leaflet ignores altitude, so the customer preview requested a 100 m plan and then drew it flat. TASK-017 adds a Cesium view to that same review step, which draws the path at whatever altitude comes back -- turning a dormant placeholder into a picture of a drone spraying from 100 m. Now `20`, naming its twins in comment. Verified safe to change: `alt_m` reaches only `coverage.py:331`, where it stamps each waypoint, so passes, flight time and therefore the customer's price estimate are all unchanged. **The general lesson is not about altitude: a placeholder is only as harmless as the least capable thing that renders it, and adding a more truthful renderer promotes every dormant lie in its inputs to a visible one.** Worth expecting the next time a 2D surface grows a 3D twin. | Claude (charlie) | closes the `web/` half of the default-twins row above |
